...
/**
 * Returns the root element which contains the given element.
 *
...
Canvas.prototype.addRootElement = function(rootElement) {
  ...
  if (!rootElement) {
    rootElement = {
      id: '__implicitroot_' + this._roots++,
      children: [],
      isImplicit: true
    };
  }
  ...
  var rootGfx = createGroup(this.getDefaultLayer(), 'djs-root');
  svgAttr(rootGfx, 'display', 'none');


  this._addRoot(rootElement, rootGfx);

  eventBus.fire('root.added', { element: rootElement, gfx: rootGfx });
...
Canvas.prototype.removeRootElement = function(rootElement) {
  ...
  // TODO(marstamm): we don't use layers, remove svg element instead
  // cleanup layer
  this._removeLayer(rootElement.id);
 ...
 */
Canvas.prototype.setRootElement = function(newRootElement, override) {

  var currentRoot = this._rootElement;

  if (newRootElement === currentRoot) {
    return;
  }

  if (!newRootElement) {
    throw new Error('newRootElement required');
  }

  if (!this._elementRegistry.get(newRootElement.id)) {
    this.addRootElement(newRootElement);
  }

  // give set add semantics for backwards compatibility
  this._setRoot(newRootElement);

  if (override && currentRoot) {
    this.removeRootElement(currentRoot);
  }
...
Canvas.prototype._setRoot = function(rootElement) {
  var currentRoot = this._rootElement;
  ...
  if (currentRoot) {
    ...
    svgAttr(this._elementRegistry.getGraphics(currentRoot), 'display', 'none');
  }
  ...
  if (rootElement) {
    ...
    // show root layer
    svgAttr(this._elementRegistry.getGraphics(rootElement), 'display', 'block');
  }
